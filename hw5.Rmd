...
```
```{r}
### Q1: Perform ARIMA on your time series and explain the output
auto_model <- auto.arima(data_ts)
summary(auto_model)

# Explanation:
# The output shows the selected ARIMA(p,d,q) model along with AIC, BIC, and log-likelihood.
# Coefficients for AR and MA terms are provided, indicating the contribution of each part.
# The value of d shows how many differences were needed to make the data stationary.

### Q2: Run Residual Analysis
checkresiduals(auto_model)

# Explanation:
# This plots residuals, ACF of residuals, and runs a Ljung-Box test.
# Good residuals should have no autocorrelation (flat ACF), and the p-value of Ljung-Box test should be > 0.05

### Q3: Forecast and plot for the next five periods
forecast_arima <- forecast(auto_model, h=5)
autoplot(forecast_arima) +
  ggtitle("5-Month Forecast from ARIMA Model") +
  xlab("Year") + ylab("Closing Price")

# Explanation:
# This forecast provides point predictions with confidence intervals for the next 5 months.

### Q4: Show accuracy of your ARIMA model
arima_accuracy <- accuracy(forecast_arima)
print(arima_accuracy)

# Explanation:
# Metrics like RMSE, MAE, and MAPE help assess how well the model fits.
# Lower RMSE and MAPE indicate better accuracy.
